...
export function ExpertReviewEditor({
  ...
  async function save(event: FormEvent) {
    ...
    if (decisionStatuses.includes(draft.status) && !canDecide) {
      setError("Only a reviewer or administrator can record a review decision.");
      return;
    }
  ...
  return (
    <form className="space-y-4" onSubmit={save}>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <FieldControl label="Reviewer name">
          <input className={inputClass} disabled={!editable} value={draft.reviewerName} onChange={(event) => updateDraft("reviewerName", event.target.value)} />
        </FieldControl>
