...
export function EvidenceEditor({ assessmentId, role, sources, onChanged }: {
  ...
  async function save(event: FormEvent) {
    ...
    if (!supabase || !editable || !draft.title.trim()) return;
    setSaving(true);
    setError("");
    ...
    if (file) {
      ...
      const storagePath = `${assessmentId}/${crypto.randomUUID()}-${safeName}`;
      ...
      await supabase.from("uploaded_files").insert({ site_assessment_id: assessmentId, file_name: file.name, document_category: "evidence", storage_path: storagePath, description: draft.summary.trim() || null });
    }
      ...
      {editable ? (
        <form onSubmit={save} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-4">
          <div className="mb-4 flex items-center justify-between gap-3">
            <h3 className="font-semibold text-[var(--color-text-primary)]">{editingId ? "Edit evidence source" : "Add evidence source"}</h3>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <FieldControl label="Evidence file"><label className={`${secondaryButtonClass} w-full cursor-pointer`}><Upload size={16} />{file?.name || "Choose file"}<input className="sr-only" type="file" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></label></FieldControl>
            <FieldControl label="Summary" wide><textarea className={textareaClass} rows={3} value={draft.summary} onChange={(event) => update("summary", event.target.value)} /></FieldControl>
            <FieldControl label="Limitations" wide><textarea className={textareaClass} rows={2} value={draft.limitationNotes} onChange={(event) => update("limitationNotes", event.target.value)} /></FieldControl>
          </div>
          {error ? <p role="alert" className="mt-3 text-sm text-[var(--color-danger)]">{error}</p> : null}
          <button type="submit" disabled={saving} className={`${primaryButtonClass} mt-4`}>{saving ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />} Save evidence</button>
        </form>
      ) : null}
        ...
        {sources.map((source) => (
          <article key={source.id} className="rounded-md border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="flex gap-2">{source.file_reference ? <button type="button" className={secondaryButtonClass} onClick={() => void download(source)}><Download size={16} /> File</button> : null}{editable ? <button type="button" className={secondaryButtonClass} onClick={() => edit(source)}><Edit3 size={16} /> Edit</button> : null}</div>
            </div>
